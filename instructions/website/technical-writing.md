...
- Put one main idea or instruction in each sentence. Use a list when several
  parallel facts would otherwise obscure the structure.
- Keep closely related cause, contrast, sequence, and consequence in the same
  paragraph. Do not split connected reasoning into abrupt statements merely
...
- Prefer concrete verbs. Avoid promotional language, rhetorical questions,
  idioms, metaphors, personification, filler, and stacked modifiers.
- Keep limits beside the capabilities they qualify. Do not remove a condition,
  ownership rule, exception, timing fact, or experimental qualification to
  make prose shorter.
